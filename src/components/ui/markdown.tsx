...
export function Markdown({ content, fontSize = "sm" }: { content: string; fontSize?: "xs" | "sm" | "md" | "lg" }) {
    ...
    <ReactMarkdown
      ...
      components={{
        ...
        ul: ({ node, children, ...props }) => (
          <ul className={`list-item list-inside pl-6 space-y-1 dark:text-gray-300 text-gray-700 ${fontSizeClasses[fontSize]}`}>
            {children}
          </ul>
        ),
        ol: ({ node, children, ...props }) => (
          <ol className={`list-decimal space-y-1 dark:text-gray-300 text-gray-700 ${fontSizeClasses[fontSize]}`}>
            {children}
          </ol>
        ),
        li: ({ node, ...props }) => (
        ...
        code({ node, inline, className, children, ...props }) {
          ...
          return !inline && match ? (
            <CodeBlock language={match[1]} value={children as string} fontSize={fontSize} />
          ) : (
            <code
              {...props}
